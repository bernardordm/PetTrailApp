...
export class FirebaseService implements OnModuleInit {
  ...
  async clearTutorTourCreatedNotification(
    tutorIdentifier: string,
  ): Promise<void> {
    await this.db
      .ref(`notifications/tutors/${tutorIdentifier}/tour_created`)
      .remove();
  }

  // Fase 2: notifica o tutor com o tour_id após criação assíncrona via fila
  async notifyTutorTourCreated(
    tutorIdentifier: string,
    data: {
      tour_id: string;
      walker_name: string;
